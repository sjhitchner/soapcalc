...
func testRecipeBatchLipidsSoftDelete(t *testing.T) {
	t.Parallel()
...
func testRecipeBatchLipidsQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := RecipeBatchLipids().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testRecipeBatchLipidsSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testRecipeBatchLipidsQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := RecipeBatchLipids().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
var (
	recipeBatchLipidDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Weight`: `double precision`, `Cost`: `double precision`, `LipidID`: `integer`}
	_                       = bytes.MinRead
)
