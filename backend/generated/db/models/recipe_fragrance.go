...
func RecipeFragrances(mods ...qm.QueryMod) recipeFragranceQuery {
	mods = append(mods, qm.From("\"recipe_fragrance\""), qmhelper.WhereIsNull("\"recipe_fragrance\".\"deleted_at\""))
...
func FindRecipeFragrance(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RecipeFragrance, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"recipe_fragrance\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *RecipeFragrance) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"recipe_fragrance\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(recipeFragranceType, recipeFragranceMapping, append(wl, recipeFragrancePrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q recipeFragranceQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o RecipeFragranceSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"recipe_fragrance\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, recipeFragrancePrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
