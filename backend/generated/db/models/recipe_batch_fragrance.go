...
var (
	recipeBatchFragranceAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "weight", "cost", "fragrance_id"}
	recipeBatchFragranceColumnsWithoutDefault = []string{"created_at", "updated_at", "deleted_at", "weight", "cost", "fragrance_id"}
...
func (recipeBatchFragranceL) LoadFragrance(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRecipeBatchFragrance interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qmhelper.WhereIsNull(`fragrance.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Adds o to related.R.RecipeBatchFragrances.
func (o *RecipeBatchFragrance) SetFragrance(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Fragrance) error {
	...
	if related.R == nil {
		related.R = &fragranceR{
			RecipeBatchFragrances: RecipeBatchFragranceSlice{o},
		}
	} else {
		related.R.RecipeBatchFragrances = append(related.R.RecipeBatchFragrances, o)
	}
...
func RecipeBatchFragrances(mods ...qm.QueryMod) recipeBatchFragranceQuery {
	mods = append(mods, qm.From("\"recipe_batch_fragrance\""), qmhelper.WhereIsNull("\"recipe_batch_fragrance\".\"deleted_at\""))
...
func FindRecipeBatchFragrance(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RecipeBatchFragrance, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"recipe_batch_fragrance\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *RecipeBatchFragrance) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"recipe_batch_fragrance\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(recipeBatchFragranceType, recipeBatchFragranceMapping, append(wl, recipeBatchFragrancePrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q recipeBatchFragranceQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o RecipeBatchFragranceSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"recipe_batch_fragrance\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, recipeBatchFragrancePrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func RecipeBatchFragranceExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"recipe_batch_fragrance\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
