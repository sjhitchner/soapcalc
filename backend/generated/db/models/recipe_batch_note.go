...
func RecipeBatchNotes(mods ...qm.QueryMod) recipeBatchNoteQuery {
	mods = append(mods, qm.From("\"recipe_batch_note\""), qmhelper.WhereIsNull("\"recipe_batch_note\".\"deleted_at\""))
...
func FindRecipeBatchNote(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RecipeBatchNote, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"recipe_batch_note\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *RecipeBatchNote) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"recipe_batch_note\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(recipeBatchNoteType, recipeBatchNoteMapping, append(wl, recipeBatchNotePrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q recipeBatchNoteQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o RecipeBatchNoteSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"recipe_batch_note\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, recipeBatchNotePrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func RecipeBatchNoteExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"recipe_batch_note\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
