...
func (o *Fragrance) FragranceInventories(mods ...qm.QueryMod) fragranceInventoryQuery {
	...
	queryMods = append(queryMods,
		...
		qmhelper.WhereIsNull("\"fragrance_inventory\".\"deleted_at\""),
	)
...
func (o *Fragrance) RecipeFragrances(mods ...qm.QueryMod) recipeFragranceQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"recipe_fragrance\".\"fragrance_id\"=?", o.ID),
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (fragranceL) LoadRecipeFragrances(ctx context.Context, e boil.ContextExecutor, singular bool, maybeFragrance interface{}, mods queries.Applicator) error {
	var slice []*Fragrance
	var object *Fragrance
...
// Sets related.R.Fragrance appropriately.
func (o *Fragrance) AddRecipeFragrances(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RecipeFragrance) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.FragranceID = o.ID
...
func Fragrances(mods ...qm.QueryMod) fragranceQuery {
	mods = append(mods, qm.From("\"fragrance\""), qmhelper.WhereIsNull("\"fragrance\".\"deleted_at\""))
...
func FindFragrance(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Fragrance, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"fragrance\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Fragrance) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"fragrance\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(fragranceType, fragranceMapping, append(wl, fragrancePrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q fragranceQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o FragranceSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"fragrance\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, fragrancePrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func FragranceExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"fragrance\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
