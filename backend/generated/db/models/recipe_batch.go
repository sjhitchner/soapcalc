...
func (o *RecipeBatch) BatchRecipeBatchNotes(mods ...qm.QueryMod) recipeBatchNoteQuery {
	...
	queryMods = append(queryMods,
		...
		qmhelper.WhereIsNull("\"recipe_batch_note\".\"deleted_at\""),
	)
...
func RecipeBatches(mods ...qm.QueryMod) recipeBatchQuery {
	mods = append(mods, qm.From("\"recipe_batch\""), qmhelper.WhereIsNull("\"recipe_batch\".\"deleted_at\""))
...
func FindRecipeBatch(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RecipeBatch, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"recipe_batch\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *RecipeBatch) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"recipe_batch\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(recipeBatchType, recipeBatchMapping, append(wl, recipeBatchPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q recipeBatchQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o RecipeBatchSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"recipe_batch\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, recipeBatchPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func RecipeBatchExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"recipe_batch\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
