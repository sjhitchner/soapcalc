...
func testRecipeBatchFragrancesSoftDelete(t *testing.T) {
	t.Parallel()
...
func testRecipeBatchFragrancesQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := RecipeBatchFragrances().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testRecipeBatchFragrancesSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testRecipeBatchFragrancesQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := RecipeBatchFragrances().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
var (
	recipeBatchFragranceDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Weight`: `double precision`, `Cost`: `double precision`, `FragranceID`: `integer`}
	_                           = bytes.MinRead
)
