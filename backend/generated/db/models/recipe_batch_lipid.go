...
var (
	recipeBatchLipidAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "weight", "cost", "lipid_id"}
	recipeBatchLipidColumnsWithoutDefault = []string{"created_at", "updated_at", "deleted_at", "weight", "cost", "lipid_id"}
...
func RecipeBatchLipids(mods ...qm.QueryMod) recipeBatchLipidQuery {
	mods = append(mods, qm.From("\"recipe_batch_lipid\""), qmhelper.WhereIsNull("\"recipe_batch_lipid\".\"deleted_at\""))
...
func FindRecipeBatchLipid(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RecipeBatchLipid, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"recipe_batch_lipid\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *RecipeBatchLipid) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"recipe_batch_lipid\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(recipeBatchLipidType, recipeBatchLipidMapping, append(wl, recipeBatchLipidPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q recipeBatchLipidQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o RecipeBatchLipidSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"recipe_batch_lipid\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, recipeBatchLipidPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func RecipeBatchLipidExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"recipe_batch_lipid\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
