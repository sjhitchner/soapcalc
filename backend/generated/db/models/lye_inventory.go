...
func LyeInventories(mods ...qm.QueryMod) lyeInventoryQuery {
	mods = append(mods, qm.From("\"lye_inventory\""), qmhelper.WhereIsNull("\"lye_inventory\".\"deleted_at\""))
...
func FindLyeInventory(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*LyeInventory, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"lye_inventory\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *LyeInventory) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"lye_inventory\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(lyeInventoryType, lyeInventoryMapping, append(wl, lyeInventoryPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q lyeInventoryQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o LyeInventorySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"lye_inventory\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, lyeInventoryPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func LyeInventoryExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"lye_inventory\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
