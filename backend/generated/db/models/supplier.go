...
func Suppliers(mods ...qm.QueryMod) supplierQuery {
	mods = append(mods, qm.From("\"supplier\""), qmhelper.WhereIsNull("\"supplier\".\"deleted_at\""))
...
func FindSupplier(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Supplier, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"supplier\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Supplier) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"supplier\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o SupplierSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"supplier\" SET %s WHERE "+
...
func SupplierExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"supplier\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
