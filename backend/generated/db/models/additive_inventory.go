...
func AdditiveInventories(mods ...qm.QueryMod) additiveInventoryQuery {
	mods = append(mods, qm.From("\"additive_inventory\""), qmhelper.WhereIsNull("\"additive_inventory\".\"deleted_at\""))
...
func FindAdditiveInventory(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*AdditiveInventory, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"additive_inventory\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *AdditiveInventory) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"additive_inventory\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(additiveInventoryType, additiveInventoryMapping, append(wl, additiveInventoryPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q additiveInventoryQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o AdditiveInventorySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"additive_inventory\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, additiveInventoryPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func AdditiveInventoryExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"additive_inventory\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
