...
func (fragranceInventoryL) LoadSupplier(ctx context.Context, e boil.ContextExecutor, singular bool, maybeFragranceInventory interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qmhelper.WhereIsNull(`supplier.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func FragranceInventories(mods ...qm.QueryMod) fragranceInventoryQuery {
	mods = append(mods, qm.From("\"fragrance_inventory\""), qmhelper.WhereIsNull("\"fragrance_inventory\".\"deleted_at\""))
...
func FindFragranceInventory(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*FragranceInventory, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"fragrance_inventory\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *FragranceInventory) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"fragrance_inventory\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(fragranceInventoryType, fragranceInventoryMapping, append(wl, fragranceInventoryPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q fragranceInventoryQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o FragranceInventorySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"fragrance_inventory\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, fragranceInventoryPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func FragranceInventoryExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"fragrance_inventory\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
